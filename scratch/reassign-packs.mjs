...
// 사용자가 지정한 완벽한 팩별 세트 매핑
// OP-01: 'R Flower' (6장) + 'SR Light' (6장)
// OP-02: 'R2 School' (6장) + 'SSR NMIXX' (6장)
// OP-03: 'SR2 Blue Valentine' (6장)
// OP-04: 'UR EPISODE 1 ZERO FRONTIER' (6장)
...
// 4개 팩의 모든 600장 카드에 packCode, packId, packName 명확히 지정
// OP-01: #001 ~ #150
// OP-02: #151 ~ #300
// OP-03: #301 ~ #450
// OP-04: #451 ~ #600
...
currentCards.forEach((card, idx) => {
  ...
  let pCode = 'OP-01';
  let pId = 'op01';
  let pName = '1탄 계승되는 의지';

  if (cardNum >= 1 && cardNum <= 150) {
    pCode = 'OP-01'; pId = 'op01'; pName = '1탄 계승되는 의지';
  } else if (cardNum >= 151 && cardNum <= 300) {
    pCode = 'OP-02'; pId = 'op02'; pName = '2탄 정점결전';
  } else if (cardNum >= 301 && cardNum <= 450) {
    pCode = 'OP-03'; pId = 'op03'; pName = '3탄 Blue Valentine';
  } else {
    pCode = 'OP-04'; pId = 'op04'; pName = '4탄 ZERO FRONTIER';
  }
...
SET_CONFIGS.forEach((conf) => {
  ...
  console.log(`Mapping ${conf.folder} -> ${conf.packCode} (${conf.packName})`);
  ...
  SIX_MEMBERS.forEach((member, mIdx) => {
    ...
    const cardTitle = `${memberKo} - ${conf.setTitle.replace(/\[|\]/g, '').replace(' NMIXX 세트 카드', '')}`;
...
console.log('Successfully written src/data/cards.ts with user pack distributions!');
