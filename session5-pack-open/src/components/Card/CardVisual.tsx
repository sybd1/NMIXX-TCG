...
const MYSTICAL_PARTICLES = [
  { left: '10%', size: 4, delay: '0s', duration: '3s', drift: '12px' },
  { left: '30%', size: 6, delay: '0.6s', duration: '4s', drift: '-8px' },
  { left: '50%', size: 5, delay: '1.2s', duration: '3.5s', drift: '16px' },
  { left: '70%', size: 7, delay: '0.4s', duration: '4.5s', drift: '-12px' },
  { left: '90%', size: 4, delay: '1.8s', duration: '3.2s', drift: '8px' },
];
...
export const CardVisual: React.FC<CardVisualProps> = React.memo(({
  ...
  const handlePointerMove = useCallback((clientX: number, clientY: number) => {
    ...
    const rotX = roundVal(((py - 50) / 2.2));
    const rotY = roundVal(-((px - 50) / 2.2));
  ...
  const rarityBorders: Record<Card['rarity'], string> = {
    ...
    LR:  isOwned ? 'border-[2.5px] border-yellow-200 shadow-[0_0_30px_rgba(254,240,138,0.85)] ring-1 ring-amber-300/60 animate-lr-aura' : 'border border-pink-950/80',
    // 🌟 MR: 묵직하고 두꺼운 최고급 티타늄 메탈 프레임
    MR:  isOwned ? 'border-[3.5px] border-slate-200 shadow-[0_0_35px_rgba(56,189,248,0.5),inset_0_0_15px_rgba(0,0,0,0.95),0_15px_35px_rgba(0,0,0,0.95)] ring-2 ring-white/60 ring-offset-2 ring-offset-black animate-mr-aura' : 'border border-yellow-950/80',
          ...
          <>
            ...
            {isOwned && isSsrPlus && (
              <div className="absolute inset-0 rounded-2xl z-26 pointer-events-none overflow-hidden">
                {['LR', 'MR'].includes(card.rarity) ? (
                  MYSTICAL_PARTICLES.map((mp, i) => (
                    <div
                      key={i}
                      className={`mystical-particle ${card.rarity === 'MR' ? 'particle-mr' : 'particle-lr'}`}
                      style={{
                        left: mp.left,
                        width: `${mp.size}px`,
                        height: `${mp.size}px`,
                        animationDelay: mp.delay,
                        animationDuration: mp.duration,
                        ['--particle-drift' as string]: mp.drift,
                      } as React.CSSProperties}
                    />
                  ))
                ) : (
