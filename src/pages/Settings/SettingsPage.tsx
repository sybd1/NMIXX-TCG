      ...
      <div className="bg-void-900/80 border border-void-800 p-6 rounded-3xl flex flex-col gap-4">
        <div className="flex flex-col gap-1.5 border-b border-white/10 pb-4">
          ...
          <p className="text-[11px] font-mono text-slate-400 leading-normal">
            * 스마트 천장: 50팩 미등장 시 SSR 등급 이상의 카드가 중복 여부와 관계없이 무작위로 등장합니다.
          </p>
        </div>

        {/* 💻 Desktop Table View (md 이상 화면에서 노출) */}
        ...
        {/* 📱 Mobile Card List View (md 미만 모바일 화면에서 노출) */}
        <div className="md:hidden flex flex-col gap-3">
          {(['C', 'UC', 'R', 'SR', 'SSR', 'UR', 'LR', 'MR'] as Rarity[]).map(r => {
            ...
            return (
              <div
                key={r}
                className="bg-black/40 border border-void-800 p-3.5 rounded-xl flex flex-col gap-2.5 shadow-sm active:bg-black/60 transition-colors"
              >
                {/* 등급 이름 & 봉입률 퍼센트 대비 배치 */}
                <div className="flex items-center justify-between border-b border-void-800/60 pb-1.5">
                  <div className="flex items-center gap-2">
                    <span
                      className={`inline-flex items-center justify-center min-w-[38px] px-2 py-0.5 rounded-md text-[10px] font-black tracking-wider border shadow-sm ${conf.badgeBg}`}
                    >
                      {r}
                    </span>
                    <span className="text-slate-200 font-serif font-bold text-xs">
                      {conf.label.replace(/^[A-Z]+\s+/, '')}
                    </span>
                  </div>
                ...
                <div className="flex flex-col gap-1 text-[11px] font-mono">
                  <div className="flex justify-between">
                    <span className="text-slate-500">가공:</span>
                    <span className="text-pink-300 font-bold">{finishConf?.nameKo || conf.finishType}</span>
                  </div>
                  <div className="flex justify-between gap-4 text-right">
                    <span className="text-slate-500 flex-shrink-0">이펙트:</span>
                    <span className="text-slate-400 font-sans leading-normal">{finishConf?.visualSpec || '표준 가공'}</span>
                  </div>
                </div>
              </div>
            );
