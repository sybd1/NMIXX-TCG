...
import { CraftingModal } from '../../components/Shop/CraftingModal';
import { sound } from '../../services/soundService';
import { ArrowUpDown, Crown, Sparkles, CheckCircle2, Lock, Landmark, PartyPopper, Hammer } from 'lucide-react';
...
export const CollectionPage: React.FC<CollectionPageProps> = ({
  ...
  const allViewableCards = MASTER_CARDS;
  const ownedLegacyCards: any[] = [];
    ...
    <div className="flex-1 max-w-[1440px] mx-auto w-full px-4 sm:px-6 md:px-8 py-8 flex flex-col gap-6">
      ...
      <div className="flex items-center gap-2 border-b border-void-800 pb-2">
        ...
        <button
          ...
          className="ml-auto flex items-center gap-2 px-5 py-2.5 rounded-2xl font-serif font-black text-xs md:text-sm tracking-wider transition-all cursor-pointer bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white shadow-lg shadow-purple-950/30 border border-purple-400/30"
        >
          <Hammer size={15} />
          더스트 공방 (제작/분해)
        </button>
                    ...
                    <div className="flex items-center gap-3.5">
                      ...
                      <div className="flex flex-col gap-0.5">
                        ...
                        <span className="text-xs font-mono text-slate-400">
                          {isCompleted 
                            ? '🎉 세트 완성 보상 코인이 지급되었습니다!' 
                            : '6명의 멤버 카드를 모두 수집하면 게임 머니 2,000,000 N COIN이 지급됩니다.'}
                        </span>
                      </div>
                    </div>
                  </div>
      ...
      <CardModal
        ...
        dust={dust}
        onCraftCard={onCraftCard}
        onDismantleSingle={onDismantleSingle}
      />

      {/* Crafting & Dismantling Modal */}
      <CraftingModal
        isOpen={isCraftingOpen}
        onClose={() => setIsCraftingOpen(false)}
        dust={dust}
        collection={collection}
        onDismantleAll={onDismantleAll}
        onCraftCard={onCraftCard}
      />
    </div>
  );
