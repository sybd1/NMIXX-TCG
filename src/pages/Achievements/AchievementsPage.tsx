          ...
          {isCompleted ? (
            isClaimed ? (
              <div className="px-3 py-1.5 rounded-xl bg-void-950 text-emerald-400 font-mono font-bold text-xs border border-emerald-500/30 flex items-center gap-1">
                ...
                <span className="hidden sm:inline">달성 완료</span>
              </div>
            ) : (
              <button
                onClick={() => onClaimReward(ach.id, ach.rewardCoins)}
                className="px-3.5 py-1.5 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-400 hover:to-teal-400 text-white font-mono font-black text-xs shadow-md shadow-emerald-950/60 flex items-center gap-1 hover:scale-105 transition-all cursor-pointer border border-white/30"
              >
