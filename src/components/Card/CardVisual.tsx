      ...
      >
        ...
        {isXrMystery ? (
          ...
          <>
            ...
            {/* ── [Layer 2: Foil & Holo Shine Layer] (z-22로 올려 인물 캐릭터 이미지 위로 빔 노출) ── */}
            {isOwned && card.rarity !== 'XR' && (
              <div className={`absolute inset-0 rounded-2xl z-22 pointer-events-none ${finishClass}`} />
            )}
            {/* ── [Layer 2-B: Holo Shine Spectrum Layer] (z-23으로 올려 인물 이미지 위에서 무지개 스펙트럼 합성) ── */}
            {isOwned && isSsrPlus && card.rarity !== 'XR' && (
              <div
                className={`card__shine absolute inset-0 rounded-2xl z-23 pointer-events-none ${
                  ['MR', 'LR'].includes(card.rarity) ? 'holo-prism-intense' : 'holo'
                }`}
                style={{
                  ['--holo-strength' as string]: ['MR', 'LR'].includes(card.rarity) ? '0.85' : '0.65',
                } as React.CSSProperties}
              />
            )}
            ...
            {card.image && (
              <div className="absolute inset-0 pointer-events-none z-20 overflow-hidden">
                <img
                  ...
                  className={`w-full h-full group-hover:scale-[1.02] transition-transform duration-200 pointer-events-none ${
                    (card.image || '').includes('FANS_1787479899008_1') || (card.image || '').includes('FANS_1787479899073_2')
                  ...
                  style={{
                    objectPosition: (card.image || '').includes('82f8e09b5737f408fc2c95c36b6ef6af')
                      ? '92% center' /* 68번 설윤: 가로 원본 우측 끝에 위치한 얼굴 노출 */
                      : (card.image || '').includes('1f8b0f6aa061e8aa61982ce844f04dd3')
                      ? '80% center' /* 80번 릴리: 가로 원본의 우측(80%)에 위치한 릴리 얼굴 노출 */
                      : (card.image || '').includes('0639f8a0ec3bee990511f0659fa2e8e5')
                      ? '42% center' /* 3번 설윤: 가로 원본의 좌측-중앙(42%)에 위치한 설윤 얼굴 노출 */
                      : (card.image || '').includes('c4b2bea5d0346a1d017086f6fe9eb143')
                      ? '20% center' /* 29번 규진: 가로 원본의 좌측(20%)에 위치한 규진 얼굴 노출 */
                      : 'center center'
                  }}
                />
              </div>
            ...
            {/* ── [Layer 4-D: LR 전용 인물 위 십자 골드 격자 크로스 빔 레이어] ── */}
            {isOwned && card.rarity === 'LR' && (
              <div className="absolute inset-0 rounded-2xl z-22 pointer-events-none finish-lr-cross-metal" />
            )}
            ...
            {showDetails && (
              <div className="relative z-30 mt-auto w-full px-3 py-2 flex items-end justify-between pointer-events-none">
                {/* 하단 좌측: 멤버 이름 (Bold Large) + NMIXX 서브텍스트 */}
                <div className="flex flex-col leading-tight overflow-hidden max-w-[65%]">
                  <span className="font-sans font-black text-[12px] sm:text-[13.5px] text-white/90 tracking-tight drop-shadow-[0_1px_2.5px_rgba(0,0,0,0.88)] truncate">
                    {isLogoCard
                      ? 'Symbol'
                      : card.member === 'NMIXX'
                      ? 'Group Portrait'
                  ...
                  <span className="font-mono font-bold text-[7px] sm:text-[7.5px] text-slate-300/80 tracking-widest drop-shadow-[0_1px_2px_rgba(0,0,0,0.85)]">
                    NMIXX
                  </span>
                </div>
                ...
                <div className="flex flex-col items-end leading-tight flex-shrink-0">
                  <span className="font-mono font-bold text-[7px] sm:text-[7.5px] text-slate-300/80 tracking-tight drop-shadow-[0_1px_2px_rgba(0,0,0,0.85)]">
                    {serialCode}
                  </span>
                </div>
              </div>
