...
import { X, Sparkles, Layers, Shield, Sparkle, Volume2, Hammer, Trash2, AlertCircle } from 'lucide-react';
...
interface CardModalProps {
  ...
  onDismantleSingle?: (cardId: string) => { success: boolean; dustGained?: number; error?: string };
}
...
export const CardModal: React.FC<CardModalProps> = ({
  ...
  // Dummy reference to satisfy compiler rules for unused variables during temporary feature bypass
  const _unused = { Hammer, Trash2, AlertCircle, dust, onCraftCard, onDismantleSingle };
  if (Object.keys(_unused).length === 0) { /* noop */ }
    ...
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 select-text">
          ...
          {/* Modal Box (반응형 크기 제어 & 스크롤 지원) */}
          <motion.div
            initial={{ scale: 0.88, opacity: 0, y: 25 }}
            animate={{ scale: 1, opacity: 1, y: 0 }}
            exit={{ scale: 0.88, opacity: 0, y: 25 }}
            transition={{ type: 'spring', damping: 25, stiffness: 300 }}
            className="relative z-10 w-full max-w-4xl lg:max-w-5xl max-h-[90vh] md:max-h-[94vh] overflow-y-auto bg-gradient-to-b from-void-900 via-void-950 to-black border-2 border-white/20 rounded-3xl p-5 sm:p-8 lg:p-10 shadow-[0_0_50px_rgba(0,0,0,0.9)] flex flex-col md:flex-row gap-5 md:gap-7 lg:gap-10 items-center scrollbar-thin scrollbar-thumb-void-800"
          >
            {/* Close Button */}
            <button
              onClick={onClose}
            ...
            {/* Left: Card Visual (반응형 크기 스케일 자동 조율) */}
            <div className="flex-shrink-0 flex justify-center py-2 w-full md:w-auto max-w-[280px] sm:max-w-none">
              <div
              ...
              >
                <CardVisual
                  ...
                  count={count}
                  size="lg"
                  className="w-56 sm:w-64 md:w-76 lg:w-[330px] h-[330px] sm:h-[380px] md:h-[460px] lg:h-[490px]"
                />
              </div>
            </div>
            ...
            <div className="flex-1 flex flex-col justify-between self-stretch gap-4 sm:gap-5">
              ...
              <div className="flex flex-col gap-2 mt-3">
                {isOwned && count > 1 && onDismantleSingle && (
                  <button
                    onClick={() => {
                      onDismantleSingle(card.id);
                    }}
                    className="w-full py-3 px-6 rounded-2xl font-mono font-black text-xs sm:text-sm bg-gradient-to-r from-red-950/80 to-rose-950/80 hover:from-red-900/95 hover:to-rose-900/95 text-rose-300 border border-rose-500/30 hover:border-rose-400 transition-all flex items-center justify-center gap-2 hover:scale-[1.01] cursor-pointer"
                  >
                    <Trash2 size={16} />
                    <span>중복 1장 분해하기 (✨ +{config.dustDismantle} DUST)</span>
                  </button>
                )}

                {!isOwned && onCraftCard && (() => {
                  const cost = config.dustCraft;
                  if (card.isSetReward || cost === null || cost === undefined) {
                    return (
                      <div className="w-full py-3 px-6 rounded-2xl font-sans text-xs text-center bg-void-950 border border-white/5 text-slate-500 flex items-center justify-center gap-2">
                        <AlertCircle size={15} />
                        <span>이 카드는 제작할 수 없는 특수 카드입니다.</span>
                      </div>
                    );
                  }
                  ...
                  return (
                    <button
                      ...
                      disabled={!canAfford}
                      className={`w-full py-3 px-6 rounded-2xl font-mono font-black text-xs sm:text-sm transition-all flex items-center justify-center gap-2 hover:scale-[1.01] cursor-pointer ${
                        canAfford
                          ? 'bg-gradient-to-r from-pink-600 to-purple-600 hover:from-pink-500 hover:to-purple-500 text-white border border-pink-400/30'
                          : 'bg-void-950 border border-void-800 text-slate-600 cursor-not-allowed border-dashed'
                      }`}
                    >
                      <Hammer size={16} />
                      <span>카드 제작하기 (✨ {cost.toLocaleString()} DUST 소모)</span>
                      {!canAfford && <span className="text-[10px] bg-red-950/80 text-red-400 px-1.5 py-0.5 rounded border border-red-500/20 ml-1 font-bold">더스트 부족</span>}
                    </button>
                  );
                })()}
              </div>
